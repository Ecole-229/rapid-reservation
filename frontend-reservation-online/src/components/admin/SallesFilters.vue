...
// Filtres avancés (dropdown)
const isAdvancedOpen = ref(false)
const minPrix = ref('')
const maxPrix = ref('')
const minCapacite = ref('')
const maxCapacite = ref('')
...
const activeAdvancedCount = () =>
  [minPrix.value, maxPrix.value, minCapacite.value, maxCapacite.value].filter(
    (v) => v !== '',
  ).length
...
const applyPriceFilter = () => {
  emit('price-change', {
    min: minPrix.value !== '' ? Number(minPrix.value) : null,
    max: maxPrix.value !== '' ? Number(maxPrix.value) : null,
  })
}

const applyCapacityFilter = () => {
  emit('capacity-change', {
    min: minCapacite.value !== '' ? Number(minCapacite.value) : null,
    max: maxCapacite.value !== '' ? Number(maxCapacite.value) : null,
  })
}
...
const resetAdvanced = () => {
  ...
  minCapacite.value = ''
  maxCapacite.value = ''
  emit('price-change', { min: null, max: null })
  emit('capacity-change', { min: null, max: null })
}
</script>
    ...
    <div class="flex w-full items-center gap-4">
      ...
      <div class="relative flex-1">
        ...
        <input
          v-model="search"
          type="text"
          placeholder="Rechercher une salle, localisation..."
      ...
      <div class="relative w-[210px]">
        ...
        >
          <div
            v-if="isAdvancedOpen"
            class="absolute left-0 top-[50px] z-50 w-[340px] overflow-hidden rounded-[12px] border border-[#C7D2FE] bg-white shadow-[0_8px_30px_-4px_rgba(79,70,229,0.15)]"
          >
            <!-- Header dropdown -->
            <div class="flex items-center justify-between border-b border-[#E2E8F0] bg-[#F5F3FF] px-4 py-3">
              <span class="text-[13px] font-semibold text-[#3730A3]">Filtres avancés</span>
              <button
                v-if="activeAdvancedCount() > 0"
                type="button"
            ...
            <div class="space-y-4 p-4">
              <!-- FILTRE PRIX -->
              <div>
                <p class="mb-2 text-[12px] font-semibold uppercase tracking-wide text-[#6366F1]">
                  💰 Tarif (FCFA)
                </p>
                <div class="flex items-center gap-2">
                  <div class="flex-1">
                    <label class="mb-1 block text-[11px] font-medium text-gray-500">Min</label>
                    <input
                      v-model="minPrix"
                      type="number"
                      min="0"
                      step="500"
                      placeholder="0"
                      class="h-[36px] w-full rounded-[8px] border border-[#E2E8F0] bg-[#F8FAFC] px-3 text-[13px] text-gray-800 outline-none transition focus:border-[#4F46E5] focus:bg-white focus:ring-2 focus:ring-[#4F46E5]/10"
                      @change="applyPriceFilter"
                    />
                  </div>
                  <div class="mt-4 text-gray-300">—</div>
                  <div class="flex-1">
                    <label class="mb-1 block text-[11px] font-medium text-gray-500">Max</label>
                    <input
                      v-model="maxPrix"
                      type="number"
                      min="0"
